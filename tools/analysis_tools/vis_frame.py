...
if __name__ == "__main__":
    ...
    info_path = 'data/nuscenes/fbocc-bevdetv2-nuscenes_infos_val.pkl'
    with open(info_path, 'rb') as f:
        dataset = pickle.load(f)
    data_infos = list(sorted(dataset['infos'], key=lambda e: e['timestamp']))
    ...
    for index in range(len(data_infos)):
        ...
        # occ_gt = np.load(os.path.join('./data/nuscenes', info['occ_path'],'labels.npz'))
        ...
        if isinstance(pred_vox, dict):
            ...
            # print(pred_vox)
            # print(pred_vox.keys()) # dict_keys(['pts_bbox', 'iou', 'pred_occupancy', 'index'])
