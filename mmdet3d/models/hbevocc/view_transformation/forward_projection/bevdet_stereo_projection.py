...
class BEVDetStereoForwardProjection(BEVDepth4D):
    def __init__(self,
        ...
        self.self_attn_forward1 = nn.ModuleList(HeightAwareDeformableAttention(q_size=(100, 100), q_in_dim=160, in_dim=160, dim=self.attn_dim1, out_dim=160, n_heads=self.attn_head, points=self.points,
        ...
        self.self_attn_forward2 = nn.ModuleList(HeightAwareDeformableAttention(q_size=(25, 25), q_in_dim=640, in_dim=640, dim=self.attn_dim2, out_dim=640, n_heads=self.attn_head, points=self.points,
                              attn_drop=0.0, proj_drop=0.1, stride=1, offset_range_factor=5, use_pe=True,
                              no_off=False, ksize=3) for _ in range(self.self_attn_layers))
    ...
    def bev_encoder(self, x):
        if self.with_specific_component('img_bev_encoder_backbone'):
            ...
            for m in self.self_attn_forward1:
                x[0] = m(x[0], x[0])
            x_down = x[0] # [b,160,100,100]
            for m in self.self_attn_forward2:
                x[2] = m(x[2], x[2])
    ...
    def extract_img_feat(self,
        ...
        feat_prev_iv = None
        for fid in range(self.num_frame-1-self.history_frame, -1, -1):
            # check if key_frame
            key_frame = fid == 0
            # get fid input
            img, sensor2keyego, ego2global, intrin, post_aug = imgs[fid], sensor2keyegos[fid], ego2globals[fid], intrins[fid], post_augs[fid]

            extra_ref_frame = fid == self.num_frame-self.extra_ref_frames-self.history_frame
            ...
            if key_frame or self.with_prev:
                ...
                if key_frame:
                    ...
                    if kwargs["stereo_feat_prev_iv"] is not None:
                        feat_curr_iv = kwargs["stereo_feat_prev_iv"]
                    else:
                        with torch.no_grad():
                            bev_feat, depth, feat_curr_iv, tran_feat, img_feats_reshape = self.prepare_bev_feat(*inputs_curr)
        ...
        return bev_feat, depth_key_frame, tran_feat_key_frame, ms_feat_key_frame, cam_params_key_frame, feat_curr_iv

    def extract_feat(self, points, img, img_metas, **kwargs):
        """Extract features from images and points."""
        voxel_feats, depth, tran_feats, ms_feat_key_frame, cam_params, feat_curr_iv = self.extract_img_feat(img, img_metas, **kwargs)
        return voxel_feats, depth, tran_feats, ms_feat_key_frame, cam_params, feat_curr_iv